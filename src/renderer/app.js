...
managedRunsViewController = createManagedRunsView({
  ...
  getSessionsForRun: (run) => managedSessionIdsForRun(run, managedSessionOwners).map((sessionId) => {
    const session = sessions.get(sessionId);
    return session ? { session, role: managedSessionOwners.get(sessionId)?.role || (sessionId === run.specSessionId ? "spec" : sessionId === run.ticketsSessionId ? "tickets" : "planner") } : null;
  }).filter(Boolean),
